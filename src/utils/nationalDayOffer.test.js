...
test("National Day offer is active only inside its 24-hour server window", () => {
  expect(isCampaignActive(campaign, Date.parse("2026-09-22T21:00:00.000Z"))).toBe(true);
  expect(isCampaignActive(campaign, Date.parse("2026-09-23T21:00:00.000Z"))).toBe(false);
});

test("offer pricing falls back to the normal 90-day price at expiry", () => {
  const plan = { id: "one_time_90", normalPriceSar: 15, campaign };
  expect(getPlanOfferPricing(plan, Date.parse("2026-09-22T22:00:00.000Z"))).toMatchObject({
    active: true,
    normalPrice: 15,
    price: 9.6,
  });
  expect(getPlanOfferPricing(plan, Date.parse("2026-09-23T21:00:00.000Z"))).toMatchObject({
    active: false,
    price: 15,
  });
});

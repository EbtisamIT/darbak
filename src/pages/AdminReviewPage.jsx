      ...
      {adminView === "analytics" ? renderSubscriptionDashboard() : adminView === "legacyAnalytics" ? (
        ...
        <div style={{ display: "grid", gap: "14px" }}>
          ...
          <form onSubmit={saveCompany} style={{ ...cardStyle, display: "grid", gap: 12 }}>
            ...
            <label style={{ display: "flex", alignItems: "flex-start", gap: 9, color: adminColors.textSoft, fontSize: 13, cursor: "pointer" }}><input type="checkbox" checked={companyForm.applicationDigestEnabled !== false} onChange={(event) => setCompanyForm((prev) => ({ ...prev, applicationDigestEnabled: event.target.checked }))} style={{ marginTop: 3 }} /><span><strong style={{ color: adminColors.text }}>إرسال ملخص يومي للطلبات الجديدة</strong><br />إيميل واحد مساءً يجمع طلبات جميع برامج الشركة.</span></label>
